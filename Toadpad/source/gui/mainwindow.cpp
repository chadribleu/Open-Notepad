...
#ifdef _DEBUG
	mClass.hIcon = LoadIconW(Application::Instance(), MAKEINTRESOURCE(IDI_APPICON));
	mClass.hIconSm = LoadIconW(Application::Instance(), MAKEINTRESOURCE(IDI_APPICON));
#else
	m_wcex.hIcon = LoadIcon(Application::Instance(), MAKEINTRESOURCE(IDI_APPICON));
	m_wcex.hIconSm = LoadIcon(Application::Instance(), MAKEINTRESOURCE(IDI_APPICON));
#endif
...
void MainWindow::InitUI()
{
	...
	// todo: remove clear default memory with EDIT_USERALLOC
...
LRESULT MainWindow::OnCustomCommand()
{
	...
	case CEM_ZOOMCHANGED:
	{
		...
		if (int count = _snwprintf_s(buffer, maxSize, _TRUNCATE, L"%d%%", static_cast<int>(mParams.lParam)) != -1)
		{
